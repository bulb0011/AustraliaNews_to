...
public class FileUtils {
     ...
     * 缓存文件根目录名
     */
    private static final String FILE_DIR = "professionalShop";
    /**
     * 上传的照片文件路径
     */
    private static final String UPLOAD_FILE = "image";
    ...
    public static String getUploadCachePath(Context context) {
        if (isSDCardExist()) {
            String path = Environment.getExternalStorageDirectory().getAbsolutePath() + File.separator + FILE_DIR + File.separator + UPLOAD_FILE + File.separator;
            File directory = new File(path);
            if (!directory.exists()) directory.mkdirs();
            return path;
        } else {
