...
public class ConversationListController implements View.OnClickListener,
    ...
    public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
        ...
        if (position > 0) {
            ...
            Conversation conv = mDatas.get(position - ConversationListFragment.HEAD_VIEW_COUNT);
    ...
    public boolean onItemLongClick(AdapterView<?> parent, View view, final int position, long id) {
        final Conversation conv = mDatas.get(position - ConversationListFragment.HEAD_VIEW_COUNT);
        if (conv != null) {
            View.OnClickListener listener = new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    ...
                    if (i == R.id.jmui_top_conv_ll) {//已经置顶,去取消
                        ...
                        mDatas.remove(position - ConversationListFragment.HEAD_VIEW_COUNT);
                        if (mDatas.size() > 0) {
                            mConvListView.setNullConversation(true);
                        } else {
                            mConvListView.setNullConversation(false);
