...
import kotlinx.android.synthetic.main.activity_life_pet_transaction_list.*
 ...
 * @date 2019/5/21
 */
class PetTransactionListActivity : BaseActivity(), OnFilterClickListener {
    class LifePageDataSource @Inject constructor(apiService: ApiService) : PageDataSource<ResultBase<PageInfoBase<LifePetTransactionInfo>>, LifePetTransactionParams>(apiService) {
        override fun fetchData(loadType: Int, handler: IDataSource.IDataSourceResultHandler<ResultBase<PageInfoBase<LifePetTransactionInfo>>>?): Observable<ResultBase<PageInfoBase<LifePetTransactionInfo>>> {
            return apiService.getLifePetTransactionList(params).compose(RxUtil.normalSchedulers())
        }
    }
    ...
    lateinit var headerAdapter: RvHeaderFootViewAdapter<LifePetTransactionInfo>
    val params = LifePetTransactionParams()
    private lateinit var typeFilter: FilterListPopupWindow


    companion object {
        fun start(context: Context) {
            val starter = Intent(context, PetTransactionListActivity::class.java)
    ...
    override fun onCreate(bundle: Bundle?) {
        ...
        setContentView(R.layout.activity_life_pet_transaction_list)
    ...
    private fun initFilter() {
        val rentalMethodList = DbHelper.getInstance().getParentCodeListAndAll(C.ParentCode.ALL_PET_TYPE, "宠物类型", "不限")
