...
/**
 * @author hdl
 * @description 我的发布财富列表(别人查看)
 * @date 2019/6/11
 */
class HomePageReleaseWealthListFragment : BaseFragment() {

    class NewsListPageDataSource @Inject constructor(apiService: ApiService) : PageDataSource<ResultBase<*>, UserOidPageParams>(apiService) {
    ...
    companion object {
        fun newFragment(userOid: String?): HomePageReleaseWealthListFragment {
            val fragment = HomePageReleaseWealthListFragment()
            val bundle = Bundle()
    ...
    val adapter by lazy { WealthBrowseCollectionLifeCommonAdapter(mContext, arrayListOf()) }
    @Inject
